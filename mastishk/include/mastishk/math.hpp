...
template<class T>
Matrix<T> multiplyMatrix(Matrix<T> left, Matrix<T> right) {
    ...
    for(size_t y = 0; y < out.cols; y++) {
        for(size_t x = 0; x < out.rows; x++) {
            ...
            for(size_t i = 0; i < left.cols; i++) {
                out.data[y * out.rows + x] += left.data[y * left.cols + i] * right.data[i * right.cols + x];
            }
        }
    }
    return out;
...
template<class T>
Matrix<T> addMatrix(Matrix<T> left, Matrix<T> right) {
    assert(("Dimentions of matrices must be equal", (left.rows == right.rows && left.cols == right.cols)));
